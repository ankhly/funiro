...
window.onload = function () {
	...
	function documentActions(e) {
		...
		if (targetElement.classList.contains('card-header__icon') || targetElement.closest('.card-header__icon')) {
			if (document.querySelector('.card-list').children.length > 0) {
				document.querySelector('.card-header').classList.toggle('_active');
			}
			e.preventDefault();
		} else if (!targetElement.closest('.card-header') && !targetElement.classList.contains('actions-products__button')) {
			document.querySelector('.card-header').classList.remove('_active');
		}

		if (targetElement.classList.contains('card-list__delete')) {
			const productId = targetElement.closest('.card-list__item').dataset.cartPid;
	...
	`;
		...
		<div class="item-products__content">
			<div class="item-products__title">${productTitle}</div>
	...
	`;
		...
		<div class="item-products__actions actions-products">
			<div class="actions-products__body">
				<a href="" class="actions-products__button button button_white">Add to cart</a>
	...
	function addToCart(productButton, productId) {
		if (!productButton.classList.contains('_hold')) {
			...
			const cart = document.querySelector('.card-header__icon');
	...
	function updateCart(productButton, productId, productAdd = true) {
		const cart = document.querySelector('.card-header');
		const cartIcon = cart.querySelector('.card-header__icon');
		const cartQuantity = cartIcon.querySelector('span');
		const cartProduct = document.querySelector(`[data-card-pid="${productId}"]`);
		...
		if (productAdd) {
			...
			<a href="" class="card-list__image _ibg">${cartProductImage}</a>
			<div class="card-list__body">
				<a href="" class="card-list__title">${cartProductTitle}</a>
				<div class="card-list__quantity">Quantity: <span>1</span></div>
				<a href="" class="card-list__delete">Delete</a>
			</div>`;
				cartList.insertAdjacentHTML('beforeend', `<li data-card-pid="${productId}" class="card-list__item">${cartProductContent}</li>`);
			} else {
				const cartProductQuantity = cartProduct.querySelector('.card-list__quantity span');
